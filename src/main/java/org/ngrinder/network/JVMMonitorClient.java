...
public class JVMMonitorClient implements Closeable {
		...
		public void init() {
			...
			if (this.mBeanClient.isConnected()) {
				...
				try {
					Set<ObjectName> nameSet = server.queryNames(new ObjectName(gcObjectNameQuery),
						null);
					...
					LOGGER.warn("Error to get object name from:{}, because:", this.host,
						e.getMessage());
			...
			try {
				if (this.mBeanClient.isConnected()) {
					...
					try {
						...
						Long minorGCCount = (Long) this.mBeanClient.getAttribute(
							this.minorGCObjName, collectionCountAttrName);
					...
					Integer threadCount = (Integer) this.mBeanClient.getAttribute(
						this.threadObjName, threadAttrName);
